...
def build_runtime_container(settings: Settings | None = None) -> ServiceContainer:
    ...
    if obj_backend == "minio":
        ...
        from graph_rag.infrastructure.persistence.azure_blob import AzureBlobObjectStore
    ...
    if vec_backend == "qdrant":
        from graph_rag.infrastructure.persistence.qdrant import QdrantChunkVectorStore
    ...
    if graph_backend == "neo4j":
        from graph_rag.infrastructure.persistence.neo4j import Neo4jGraphStore
    ...
    if meta_backend in {"postgres", "postgresql", "pg"}:
        ...
        from graph_rag.infrastructure.persistence.postgres import (
            LockedAsyncProxy,
        ...
        from graph_rag.infrastructure.persistence.postgres.repositories.parsing_audit import (
            SqlAlchemyParsingAuditRepository,
        )
        from graph_rag.infrastructure.persistence.postgres.repositories.usage import (
    ...
    if resolved.security.auth_enabled:
        ...
        from graph_rag.domain.auth.passwords import is_weak_jwt_secret
...
def _wire_ingest_queue(
    ...
    *,
    ...
    from graph_rag.infrastructure.persistence.dead_letters import (
        InMemoryDeadLetterStore,
        SqlAlchemyDeadLetterStore,
    )
    from graph_rag.infrastructure.persistence.outbox import (
        InMemoryOutboxStore,
        SqlAlchemyOutboxStore,
    )
    from graph_rag.infrastructure.persistence.postgres import LockedAsyncProxy
    ...
    if redis_client is not None:
        from graph_rag.infrastructure.persistence.redis import RedisStreamIngestionQueue
